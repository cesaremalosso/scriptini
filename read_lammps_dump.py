...
class LAMMPS_Dump(object):
   ...
   def __init__(self, *args, **kwargs):
      ...
      self.line_to_skip = 0  # number of line to skip a timestep
      self._open_file()
   ...
   def _read_ckeys(self, group_vectors=True, preload_timesteps=True):
      ...
      self.select_timesteps = self.FIRST_TIMESTEP
   ...
   def _gototimestep(self, start_step, fast_check=True):
      ...
      if (start_step >= 0):
         ...
         while True:
            #for i in range(self.line_to_skip):
             #  line = self.file.readline()
            line = self.file.readline()
            if len(line) == 0:  # EOF
   ...
   def read_timesteps(self, selection, start_step=-1, select_ckeys=None, fast_check=True):
      ...
      Examples:
            ...
            read_timesteps((10,30))      -->>  Read from TIMESTEP 10 to 30, 30 excluded
   ...
   def write_Timestep_file(self,ofilename='TimestepList.dat'):
      "Write a file containing a list of all the timestep inside the dump file"
      np.savetxt(ofilename,self.timestep.T,fmt='%5d',header='List of Timestep')
      return
